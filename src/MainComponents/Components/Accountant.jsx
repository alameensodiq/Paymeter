...
const Accountant = ({ title }) => {
  ...
  const [role5, setRole5] = useState(true);
  ...
  useEffect(() => {
    if (sessionStorage.getItem("token") && role === "ACCOUNTANT") {
      dispatch(ApiAgentRole({ role5 }));
      return;
    } else {
      navigate("/");
      toast.error("You aren't logged in");
    }
    if (reload && role === "ACCOUNTANT") {
      dispatch(ApiAgentRole({ role5 }));
      setReload(false);
    }
