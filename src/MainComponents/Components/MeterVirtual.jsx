...
import { Virtualmeter } from "../Store/Apis/Virtualmeter";

const MeterVirtual = ({ title }) => {
  ...
  useEffect(() => {
    if (sessionStorage.getItem("token") && id) {
      dispatch(Virtualmeter({ meterNumber: id }));
      return;
    } else {
      navigate("/");
  ...
  const { virtual, authenticatingvirtual } = useSelector(
    (state) => state?.virtual
  );
  console.log(virtual);
  ...
  const next = virtual?.data?.meta?.next;
  const previous = virtual?.data?.meta?.prev;
  const totalPosts = virtual?.data?.meta?.totalCount;
        ...
        <div className="w-[100%] py-9 px-5 flex flex-col gap-10">
          ...
          <div className="flex flex-row justify-between">
            <span className="text-route-name text-[25px] font-semibold">
              {/* {virtual?.data?.data[0]?.customerName}Meters */}
            </span>
