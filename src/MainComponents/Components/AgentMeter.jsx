...
import { Agentmetering } from "../Store/Apis/Agentmetering";

const AgentMeter = ({ title }) => {
  ...
  useEffect(() => {
    if (sessionStorage.getItem("token")) {
      dispatch(Agentmetering({ searcher, currentPage }));
      return;
    } else {
      navigate("/");
  ...
  const next = agentmeters?.data?.meta?.next;
  const previous = agentmeters?.data?.meta?.prev;
  const totalPosts = agentmeters?.data?.meta?.totalCount;
